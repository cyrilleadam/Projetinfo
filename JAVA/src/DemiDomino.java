...
public class DemiDomino {
	...
	public boolean[] regarderAutour(Plateau plateau, int x, int y) {
		int[][] listAutour = { { 0, 1 }, { 0, -1 }, { 1, 0 }, { -1, 0 } };// création position de vérification
		...
		listeResultats[0] = true;//Droit de placer la liste domino à cet endroit
		for (int i = 0; i < listAutour.length; i++) {
			try {
				...
				if (type.equals(typeTerrain) || type.equals("C")) {//regarder si le typeTerrain est le meme type que le domino ou "C"
